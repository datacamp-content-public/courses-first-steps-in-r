...
ex() %>% check_output(848000, fixed=TRUE, missing_msg= "So ist das nicht ganz richtig - Sie müssen sich verrechnet haben und beachten Sie auch mögliche Tippfehler!")
...
success_msg("Ja, genau. Die Funktion str() ist sehr hilfreich und verschafft Ihnen einen guten Überblick über den Vektor. Es ist nützlich, die Funktion im Kopf zu behalten!")
```
...
`@sct`
```{r}
ex() %>% check_code("1", fixed=TRUE, missing_msg="Leider nicht richtig, es kommen in dem Datensatz keine boolschen Werte vor11!")
