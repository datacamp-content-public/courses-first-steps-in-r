...
`@sct`
```{r}
ex() %>% check_code("rbind(sell.time, revenue.day)", fixed=TRUE, missing_msg="Verwenden Sie bitte die Funktionen aus der Kontextbeschreibung!") 
...
`@solution`
```{r}
report.final <- rbind(sell.time, revenue.day, average.byday) | report.final <- rbind(report.wochenverkaeufe, average.byday) 
```
